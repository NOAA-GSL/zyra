...
def register_cli(subparsers: Any) -> None:
    ...
    def _viz_vector_cmd(ns):
        from datavizhub.visualization.vector_field_manager import VectorFieldManager

        mgr = VectorFieldManager(basemap=ns.basemap, extent=ns.extent, color=getattr(ns, "color", "#333333"), density=getattr(ns, "density", 0.2), scale=getattr(ns, "scale", None), streamlines=getattr(ns, "streamlines", False))
        features = _features_from_ns(ns)
        # Vector manager ignores features in render but basemap handles drawn features
        mgr.render(
