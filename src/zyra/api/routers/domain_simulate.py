...
router = APIRouter(tags=["simulate"], prefix="")


SimulateRequest = DomainRunRequest


@router.post("/simulate", response_model=DomainRunResponse)
def simulate_run(
    req: SimulateRequest, bg: BackgroundTasks, request: Request
) -> DomainRunResponse:
    """Run a simulate-domain tool (skeleton stage)."""
    try:
    ...
    if req.tool not in allowed:
        return domain_error_response(
            ...
            message="Invalid tool for simulate domain",
