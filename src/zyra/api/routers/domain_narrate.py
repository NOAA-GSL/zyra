...
router = APIRouter(tags=["narrate"], prefix="")


NarrateRequest = DomainRunRequest


@router.post("/narrate", response_model=DomainRunResponse)
def narrate_run(
    req: NarrateRequest, bg: BackgroundTasks, request: Request
) -> DomainRunResponse:
    """Run a narrate-domain tool (skeleton stage)."""
    try:
    ...
    if req.tool not in allowed:
        return domain_error_response(
            ...
            message="Invalid tool for narrate domain",
