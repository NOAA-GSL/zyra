...
def test_env_interpolation_expands_and_strict_errors(tmp_path: Path, monkeypatch):
    cfg = {
        ...
        "stages": [
            ...
            {"stage": "decimation", "command": "local", "args": {"input": "-", "path": "${TMPDIR}/out.nc"}},
        ],
    }
    ...
    assert str(tmp_path) in " ".join(items[1]["argv"]) or str(tmp_path) in json.dumps(items[1])

    # Default expansion: when VAR is missing, use provided default
    cfg_def = {
        "name": "env-default",
        "stages": [
            {"stage": "decimation", "command": "local", "args": {"input": "-", "path": "${NO_SUCH_VAR:-fallback.bin}"}},
        ],
    }
    ...
    assert "fallback.bin" in " ".join(items_def[0]["argv"]) or "fallback.bin" in json.dumps(items_def[0])

    # Strict mode should fail when a var is missing without default
    cfg2 = {
        "name": "env-strict",
        "stages": [
            {"stage": "decimation", "command": "local", "args": {"input": "-", "path": "${NOT_SET}/file.bin"}},
        ],
    }
...
def test_runner_verbosity_flags_print_headings(tmp_path: Path):
    cfg = {
        "name": "verbosity",
        "stages": [
            {"stage": "processing", "command": "convert-format", "args": {"file_or_url": "-", "format": "netcdf"}},
        ],
    }
    p = tmp_path / "pipe.json"
    p.write_text(json.dumps(cfg), encoding="utf-8")
