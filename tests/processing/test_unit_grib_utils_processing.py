...
def test_variable_not_found_error():
    ...
    from datavizhub.processing.grib_utils import DecodedGRIB, extract_variable, VariableNotFoundError
...
def test_cdo_netcdf_to_grib2(monkeypatch):
    ...
    # Pretend CDO exists
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/cdo" if name == "cdo" else None)
